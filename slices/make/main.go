...
func getMessageCosts(messages []string) []float64 {
	messageCostsSlice := make([]float64, len(messages))

	for i := 0; i < len(messages); i++ {
		messageCostsSlice[i] = float64(len(messages[i])) * 0.01
	}

	/*
		or use for range:
		  for i, message := range messages {
	        messageCostsSlice[i] = float64(len(message)) * 0.01
	    }

	*/

	return messageCostsSlice
